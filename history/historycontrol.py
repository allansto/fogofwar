...
class Controller(threading.Thread):
    ...
    def run(self):
        ...
        self.history.execute(
            "CREATE TABLE IF NOT EXISTS history (timestamp REAL,address TEXT,author TEXT,message TEXT)"
        )
        self.conn.commit()
        while True:
    ...
    def addToDatabase(self, timestamp, address, author, message):
        self.history.execute(
            "INSERT INTO history (timestamp,address,author,message) VALUES (?,?,?,?)",
            (timestamp, address, author, message),
        )
        self.conn.commit()
        return
    ...
    def getAddressCount(self, address):
        ...
        try:
            num = con.execute(
                "SELECT count(*) FROM history WHERE address = ?", (address,)
            ).fetchone()
        except:
            num = 0
            pass
    ...
    def getAuthorsByAddress(self, address):
        ...
        try:
            authors = con.execute(
                "SELECT DISTINCT author FROM history WHERE address = ?", (address,)
            ).fetchall()
        except:
            authors = ""
            pass
