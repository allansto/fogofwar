...
def main():
	...
	sh = logging.handlers.SysLogHandler(address='/dev/log',facility=config.get("LOGS","facility"))
	...
	try:
		...
		control = fogcontrol.Controller(config=config,msgqueue=msgqueue,log=log,tables=tables,history=history)
		...
		for port in ports:
			...
			try:
				...
				log.error('FogofWar: Error Could not start thread %s: %s port %s' % (str(e),"", port))
				pass
