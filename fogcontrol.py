...
class Controller(threading.Thread):
	...
	def connect(self,msg):
		...
		# QBGP active?
		if self.config.get("QBGP","qbgp") == "on" and not self.is_whitelisted(sip,self.config):
			payload = {}
			...
			try:
				r = self.sess.put("https://%s:%s@%s:%s/quarantine"%(self.config.get("QBGP","username"),self.config.get("QBGP","password"),self.config.get("QBGP","host"),self.config.get("QBGP","port")), verify=False, data=json.dumps(payload))
				self.log.debug("FogofWar: QBGP %s" % (json.dumps(payload)))
				self.log.debug("FogofWar: QBGP Response %s" % (r.json()))
				self.log.info("FogofWar: QBGP [%s] in %s seconds" % (sip,r.json()["ElapsedTime"]))
			except:
				pass
		...
		#Log it.
		self.DedupAdd(sip)
		self.log.info("FogofWar: Banned [%s]:%s to %s:%s" % (sip,sport,self.sensorname,dport))
		return
...
#
	...
	def DedupInit(self):
		...
		self.dedup.execute('CREATE TABLE IF NOT EXISTS dedup (timestamp REAL, address TEXT)')
		self.conn.commit()
		return
	...
	def DedupAdd(self,ip):
		now = time.time()
		self.dedup.execute('INSERT into dedup (timestamp,address) values (?,?)',(now,str(ip)))
		self.conn.commit()
		return
