...
class SocketListener((socketserver.BaseRequestHandler)):
    ...
    def setup(self):
        ...
        self.fake_string = "".join(
            random.choice(string.printable) for _ in range(self.length)
        )
        try:
        ...
        self.msgqueue.put(
            "Connect: %s %s %s %s"
            % (self.sip, self.sport, socket.gethostname(), self.dport)
        )
        self.log.debug(
            "FogofWar: Connection [%s]:%s to [%s]:%s"
